...
namespace SysDVR.Client.Targets.RTSP
{
    ...
    public static class H264Packetizer
    {
        ...
        private static byte[] PacketizeSingleNAL(Span<byte> raw_nal, uint rtp_timestamp)
        {
        ...
        private static void PacketizeNAL_FUA(ref List<byte[]> rtp_packets, Span<byte> raw_nal, uint rtp_timestamp)
        {
            int start_bit = 1;
            int end_bit = 0;
        ...
        public static List<byte[]> PacketizeSingleNAL(Span<byte> raw_nal, ulong tsMsec)
        {
            ...
            uint rtp_timestamp = (uint)tsMsec * 90; // 90kHz clock
