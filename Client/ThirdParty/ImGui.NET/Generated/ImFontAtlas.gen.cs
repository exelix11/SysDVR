...
namespace ImGuiNET
{
    ...
    public unsafe partial struct ImFontAtlasPtr
    {
        ...
        public unsafe ImFontPtr AddFontFromMemoryTTF(byte[] font_data, float size_pixels)
        {
            ImFont* ret = null;
            fixed (byte* data = font_data)
            {
                ImFontConfig* font_cfg = null;
                ushort* glyph_ranges = null;
                ret = ImGuiNative.ImFontAtlas_AddFontFromMemoryTTF((ImFontAtlas*)(NativePtr), data, font_data.Length, size_pixels, font_cfg, glyph_ranges);
            }
            return new ImFontPtr(ret);
        }
