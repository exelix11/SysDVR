...
static u32 USB_WaitForInputReq(UsbInterface* dev)
{
	...
	{
		u32 initSeq = 0;
		if (UsbSerialRead(dev, &initSeq, sizeof(initSeq), 1E+8) == sizeof(initSeq))
			return initSeq;
		svcSleepThread(1E+9);
...
static void USB_SendStream(GrcStream stream, UsbInterface *Dev)
{
	...
	if (*size)
	{
		...
		if (UsbSerialWrite(Dev, TargetBuf, *size, 5E+8) != *size) // 1/2 second 
			return;
	}
	return;
}
