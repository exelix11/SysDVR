...
If you really need other sysmodules at the same time you can try using the USB-Only version as it uses less memory (300K vs 1.1MB)\
...
- You can pipe SysDVR-Client stream output to any player using the `--stdout` option, this only supports one channel. Regular messages will be printed to the standard error.
